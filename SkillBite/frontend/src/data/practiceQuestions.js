...
function buildQuestion(topicLabel, title, idx, languageLabel, topicBank) {
  ...
  const fullDesc = `Your task is to implement a solution for the "${title}" problem using ${languageLabel}.\n\n${td.desc}\n\nThis module focuses on practical application of core concepts to build real-world software engineering skills. The logic should process the provided input sequence through the lens of the ${topicLabel} framework. Always ensure proper memory management and handle potential edge cases related to large input sizes.`

  return {
    title,
    difficulty,
    description: td.desc,
    inputFormat: td.input,
    outputFormat: td.output,
    input: sample.input,
    output: sample.output,
    constraints: `${td.constraints}\nLanguage: ${languageLabel} specific syntax is required.`,
    explanation: `The logic processes the provided input sequence through the lens of the ${topicLabel} framework. For the sample input, the algorithm generates the target output by applying optimized logic relevant to the ${topicBank} category.`,
    notes: `Always ensure proper memory management and handle potential edge cases related to large input sizes.`
  }
}
